...
import {sleep} from '/app/main.mjs';
...
export default async (oldState, newState) => {
      ...
      if(guild_id === newState.guild.id){
        for(let j=0; j<obj[i+1].length; j += 2){
          ...
          if(member_id === newState.member.id){
            ...
            const dataStr = objStr.replace(String(obj[i+1][j+1][0]),String(base_time))
  ...
  if (oldState.channelId !== null && newState.channelId === null){
      ...
      if(guild_id === oldState.guild.id){
        for(let j=0; j<obj[i+1].length; j += 2){
          ...
          if(member_id === newState.member.id){
            ...
            let dis_time;
            if(dis_hour === 0){
              dis_time = `<@${member_id}>が${dis_min}分、ボイスチャンネルに参加しました。`
            }else{
              dis_time = `<@${member_id}>が${dis_hour}時間${dis_min}分、ボイスチャンネルに参加しました。`
            }
            //jsonデータ処理を下に書く
            ...
            const channel = oldState.guild.channels.cache.find(
              (ch) => ch.name === "general" || ch.name === "一般"
            );
            channel.send("テストしてます。なんかすいません。\n"+dis_time)
            //3分後に再起動
    ...
    //}catch(error){
     // console.error("なんかすいません。",error.message)
    //}
  }//endpoint.
  }
  function reload() {
